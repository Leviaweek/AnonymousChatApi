    ...
    IDbContextFactory<AnonymousChatDbContext> contextFactory)
{
    ...
    public async ValueTask<bool> TryReadMessagesAsync(long userId, long chatId, long lastReadId, CancellationToken cancellationToken)
    {
        ...
        var chatUser = await db.ChatUsers.Where(user => !user.Chat.IsDeleted)
            .FirstOrDefaultAsync(chatUser => chatUser.UserId == userId, cancellationToken: cancellationToken);
        ...
        var messages = db.MessageBases
            .Where(message => message.ChatId == chatId &&
                              ...
                              message.Id >= chatUser.LastReadMessageId &&
                              message.Id <= lastReadId)
            .OrderByDescending(m => m.Id);

        var last = messages.First();

        chatUser.LastReadMessageId = last.Id;
